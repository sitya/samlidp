...
class sspmod_sqlauth_Auth_Source_SQL extends sspmod_core_Auth_UserPassBase
{
    ...
    protected function login($username, $password)
    {
        ...
        $sspgetter->addIdpAuditRecord($_SERVER['HTTP_HOST'], $username, 'none');
        ...
        if (isset($attributes['affiliation'])) {
            $member = false;
            foreach ($attributes['affiliation'] as $affiliation) {
                if (in_array($affiliation, array('faculty','staff','student','employee'))) {
                    $member = true;
                }
            }
            if ($member) {
